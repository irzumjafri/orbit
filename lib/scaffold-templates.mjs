...
export function buildAutoRunMd() {
  return `
---
description: Orbit Auto Runner — triage GitHub open issues via MCP and work one issue
---

# Auto-run (Orbit)

Triggered by Orbit Auto Runner or manual \`/auto-run\`. Use **GitHub MCP only** for issue discovery and updates.

## Rules

1. List **open** issues for repositories the user can access (newest first unless the user specified a repo).
2. **Skip** any issue that already has label \`orbit:auto-in-progress\` or \`orbit:auto-done\` (create labels via GitHub MCP if missing).
3. Pick **one** eligible issue. If none remain, reply: \`No eligible open issues for auto-run.\` and stop.
4. On the chosen issue:
   - Add label \`orbit:auto-in-progress\`
   - Comment: \`Orbit auto-run started.\`
5. Implement the fix or feature on a branch \`fix/\` or \`feat/\` per global GEMINI rules (never commit directly on \`main\`).
6. When done (PR opened or you must stop): remove \`orbit:auto-in-progress\`, add \`orbit:auto-done\` when appropriate, and leave a one-line outcome comment.
7. **Do not** start a second issue in this session.

## Response

Keep the final message short: issue #, repo, branch name, and PR link if any.
`.trim();
}

export function buildIssueMd() {
  return `
---
description: Create a highly-detailed GitHub issue from user description for Autopilot to process later
---

# Create Issue (/issue)

Triggered by \`/issue <description>\`. Use the **GitHub MCP** to create a well-documented issue in the repository.

## Rules

1. **Determine Repository**: Find the target repository from the active project workspace.
2. **Analyze User Input**: Extract the core feature request, bug details, or tasks from the user's description.
3. **Draft Detailed Documentation**: Write a comprehensive description for the GitHub issue. It must include:
   - **Context**: A brief explanation of the goal.
   - **Requirements Checklist**: A bulleted list of specific changes or requirements.
   - **Implementation Blueprint**: Suggested files to modify/create, logic changes, and styling requirements.
   - **Verification Steps**: Concrete instructions on how the agent should verify the fix (e.g. commands to run, visual checks).
4. **Create the Issue**:
   - Title format: \`[Autopilot] <short descriptive title>\`
   - Body: Use the detailed documentation drafted in Step 3.
   - Call the GitHub MCP to create the issue in the target repository.
5. **Response**: Reply to the user with the issue number and a clickable link to the created GitHub issue.
`.trim();
}
