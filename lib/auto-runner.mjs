...
import { openAntigravityWithCdp } from '../electron/antigravity-runtime.mjs';
...
export function normalizeAutoRunnerConfig(raw) {
  ...
  return {
    ...
    projectPriority: Array.isArray(r.projectPriority) ? r.projectPriority : []
  };
}
...
export class AutoRunnerController {
  ...
  constructor(opts) {
    ...
    this.readWorkspaceBaseDir = opts.readWorkspaceBaseDir || (() => '');
  ...
  async preparePriorityProject() {
    const cfg = normalizeAutoRunnerConfig(this.readAutoRunnerConfig());
    const ws = this.readWorkspaceBaseDir ? this.readWorkspaceBaseDir() : '';
    if (!ws || !fs.existsSync(ws) || !cfg.projectPriority?.length) {
      return null;
    }
    
    for (const proj of cfg.projectPriority) {
      const projPath = path.join(ws, proj);
      if (fs.existsSync(projPath)) {
        this._status.line = `Opening priority project: ${proj}…`;
        const platform = os.platform();
        try {
          await openAntigravityWithCdp(platform, projPath);
          for (let i = 0; i < 7; i++) {
            const port = await this.findCdpPort();
            if (port) break;
            await new Promise((r) => setTimeout(r, 1000));
          }
          return projPath;
        } catch (e) {
          console.error(`Failed to auto-open project ${proj}:`, e);
        }
      }
    }
    return null;
  ...
  async executeRun({ trigger, model, fireKey }) {
    ...
    await this.preparePriorityProject();

    const cdp = await this.findCdpPort();
    if (!cdp) {
      const msg = 'Antigravity CDP not available — failed to connect to priority project.';
